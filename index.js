firebase.auth().onAuthStateChanged(function(user) {
    if (user) {
      ...
      if(user != null){
        ...
        ;
        firebase.database().ref('/hospitals/'+name).on('value',function(snapshot){
...
function handleSignUp() {
  ...
  firebase.database().ref('/hospitals/'+name).set({
    name: name,
    email: email,
    mobile: mobile,
  ...
  function showPosition(position){
    ...
    ref.on("value", function(snapshot) {
      ...
      document.getElementById('patient_x_1').innerHTML=x;
      document.getElementById('patient_y_1').innerHTML=y;
